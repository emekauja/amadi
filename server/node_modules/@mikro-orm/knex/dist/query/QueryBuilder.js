...
class QueryBuilder {
    ...
    count(field, distinct = false) {
        this._fields = [...(field ? core_1.Utils.asArray(field) : this.metadata.get(this.entityName).primaryKeys)];
        if (distinct) {
            this.flags.add(core_1.QueryFlag.DISTINCT);
        }
    ...
    where(cond, params, operator) {
        cond = core_1.SmartQueryHelper.processWhere(cond, this.entityName, this.metadata.get(this.entityName, false, false));
    ...
    getKnexQuery() {
        ...
        core_1.Utils.runIfNotEmpty(() => qb.orderBy(this.helper.getQueryOrder(this.type, this._orderBy, this._populateMap)), this._orderBy);
    ...
    getNextAlias(prefix = 'e', increment = true) {
    ...
    async execute(method = 'all', mapResults = true) {
        ...
        if (method === 'all' && Array.isArray(res)) {
            return res.map(r => this.driver.mapResult(r, meta, this._populate, this._aliasMap));
        }
        return this.driver.mapResult(res, meta, this._populate, this._aliasMap);
    }
    async getResult() {
    ...
    joinReference(field, alias, cond, type, path) {
        ...
        cond = core_1.SmartQueryHelper.processWhere(cond, this.entityName, this.metadata.get(this.entityName));
    ...
    wrapPaginateSubQuery(meta) {
        ...
        this.select(this._fields).where({
            [core_1.Utils.getPrimaryKeyHash(meta.primaryKeys)]: { $in: subSubSubQuery },
        });
    }
